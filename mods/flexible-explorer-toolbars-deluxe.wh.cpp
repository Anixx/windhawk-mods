...
void AddOrUpdateToolbarMenuItems(HMENU hMenu) {
    if (!MenuHasItem(hMenu, CMD_TOGGLE_SEARCHBAND)) {
        ...
        InsertMenuW(hMenu, 0, MF_BYPOSITION | MF_STRING, CMD_TOGGLE_UPBUTTON,   sUp.c_str());
        InsertMenuW(hMenu, 0, MF_BYPOSITION | MF_STRING, CMD_TOGGLE_BREADCRUMB, sBread.c_str());
        InsertMenuW(hMenu, 0, MF_BYPOSITION | MF_STRING, CMD_TOGGLE_SEARCHBAND, sSearch.c_str());
    }
...
void EnableBand(HWND cab, BandType type) {
    ...
    // Нет сохранённой позиции -> тулбар должен начинать новую строку.
    bool wantBreak = hasSaved ? bs.brk : true;

    if (type == BandType::UpButton) ResizeUpButtonToolbar(child);
    else if (type == BandType::Breadcrumb) SendMessage(child, TB_AUTOSIZE, 0, 0);
    ...
    if (wantBreak) rbi.fStyle |= RBBS_BREAK;
    rbi.hwndChild = child;
...
bool DoMoveSearchBandToMenuBar(HWND cabinetWnd){
    ...
    // Регистрируем связку cabinet<->menuRebar ВСЕГДА, даже если сейчас
    // нечего переносить (например, все опциональные тулбары выключены).
    // Иначе последующее включение тулбара через контекстное меню не
    // сможет найти нужный rebar до переоткрытия окна.
    {
    ...
    if(!toMove.empty()){
        ...
        for(auto&b:toMove){
            ...
            // Нет сохранённой позиции -> ставим тулбар на новую строку.
            bool wantBreak=hasSaved?bs.brk:true;
