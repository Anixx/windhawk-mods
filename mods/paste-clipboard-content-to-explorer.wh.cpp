...
// ============================================================
//  Найти активный ShellView
// ============================================================
static HWND FindActiveShellView()
{
    HWND fore = GetForegroundWindow();
    if (!fore) return nullptr;
    
    // Проверяем само окно
    if (IsShellViewWindow(fore))
        return fore;
    ...
    EnumChildWindows(fore, CB::Enum, (LPARAM)&fd);
    return fd.defView;
}
...
BOOL WINAPI EnableMenuItem_Hook(HMENU hMenu, UINT uIDEnableItem, UINT uEnable)
{
    if (IsPasteCmd(uIDEnableItem) && (uEnable & (MF_GRAYED | MF_DISABLED)))
    {
        ...
        if (!hwnd) hwnd = FindActiveShellView(); // Добавлено: для выпадающего меню
...
// Добавлено: GetMenuItemInfoW для выпадающего меню
using GetMenuItemInfoW_t = BOOL(WINAPI*)(HMENU, UINT, BOOL, LPMENUITEMINFOW);
GetMenuItemInfoW_t GetMenuItemInfoW_Orig;
BOOL WINAPI GetMenuItemInfoW_Hook(HMENU hMenu, UINT uItem, BOOL fByPosition,
                                   LPMENUITEMINFOW lpmii)
{
    BOOL result = GetMenuItemInfoW_Orig(hMenu, uItem, fByPosition, lpmii);
    if (!result || !lpmii) return result;

    // Определяем ID пункта
    UINT cmdId = 0;
    if (fByPosition)
    {
        MENUITEMINFOW tmp = {};
        ...
        tmp.fMask  = MIIM_ID;
        if (GetMenuItemInfoW_Orig(hMenu, uItem, TRUE, &tmp))
            cmdId = tmp.wID;
    }
    else
    {
    ...
    if (IsPasteCmd(cmdId) && (lpmii->fMask & MIIM_STATE))
    {
        HWND hwnd = GetShellViewWindowAtCursor();
        if (!hwnd) hwnd = g_lastContextMenuShellView;
        if (!hwnd) hwnd = FindActiveShellView();
        
        if (hwnd && CanPasteInWindow(hwnd))
            lpmii->fState = (lpmii->fState & ~(MFS_GRAYED | MFS_DISABLED)) | MFS_ENABLED;
    }
    return result;
}
                                   ...
                                   int x, int y, HWND hwnd, LPTPMPARAMS lptpm)
{
    ...
    if (canPaste)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (GetMenuItemInfoW_Orig(hmenu, (UINT)i, TRUE, &mii) &&
                IsPasteCmd(mii.wID))
            {
...
BOOL Wh_ModInit()
{
    ...
    Wh_SetFunctionHook((void*)GetMenuItemInfoW,
                       (void*)GetMenuItemInfoW_Hook,
                       (void**)&GetMenuItemInfoW_Orig);
